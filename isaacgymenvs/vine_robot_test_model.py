...
class VineRobotControlModel(nn.Module):
    def __init__(self, file_path, x_range, u_range):
        super().__init__()
        self.file_path = file_path
        self.nn_model = torch.load(file_path).model  # TODO: See if need to set device
...
def create_env_thunk(**kwargs):
    import isaacgymenvs
    BIG_CFG_PATH = "runs/Vine5LinkMovingBase/config.yaml"  # Config of the model
    from omegaconf import OmegaConf
    cfg = OmegaConf.load(os.path.join(FOLDER, BIG_CFG_PATH))
...
player = players.PpoPlayerDiscrete(cfg['params'])
_restore(player, {"checkpoint": os.path.join(FOLDER, CHECKPOINT_PATH)})
...
class BasePlayer(object):
    def __init__(self, params):
        ...
        self.states = None
        self.is_determenistic = True # TODO
